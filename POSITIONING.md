...
storage), and production scale. A significant portion of the remaining ~117 MCP tools — the non-governance
...
2. **The governance plane on the MCP surface.** Firewall · SDN/network · cluster-HA/migration ·
   ...
   **live-proven to execute** against a real PVE 9.2 API (including a nested 3-node test cluster), with PROVE
   ...
   live-migration (shared storage), and production load. A portion of the broader ~117-tool surface still
