...
| **Network attacker** (A2A and/or HTTP face enabled) | Hits either optional network face — both are full control planes; DNS-rebind; loopback-CSRF (a web page the operator loads forging a POST to `127.0.0.1`); header smuggling | One shared fail-closed perimeter (`proximo.webguard`) so the faces cannot drift apart: non-localhost binds refuse to start without a bearer token (`PROXIMO_A2A_TOKEN_FILE` / `PROXIMO_HTTP_TOKEN_FILE`, constant-time compare); Host allowlist against DNS-rebind (`PROXIMO_A2A_ALLOWED_HOSTS` / `PROXIMO_HTTP_ALLOWED_HOSTS`); a cross-origin guard on every mutating POST (Sec-Fetch-Site + Origin + Content-Type checks). Both faces dispatch through the same governed spine as stdio — PLAN/PROVE and the Layer-1 token floor apply identically; no second mutate path |
